...
struct backSubstituteTraits {
    ...
    static const int UNROLL_FACTOR =
                2; // Specify the unrolling factor
};
...
void backSubstituteAlt(const InputType A[RowsColsA][RowsColsA], OutputType B[RowsColsA][RowsColsA], int& is_singular) {
    ...
    //modified
#pragma HLS ARRAY_PARTITION variable = row_sum cyclic dim = 2 factor = 2
...
#pragma HLS ARRAY_PARTITION variable = B cyclic dim = 2 factor = TRAITS::UNROLL_FACTOR
